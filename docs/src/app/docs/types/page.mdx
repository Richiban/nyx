...
Nyx features a strong static type system with Hindley-Milner type inference. Types are descriptive and readable, and you can use record types for structure and tag unions for variants.

## Type Inference

Nyx can infer most types automatically:
...
Nyx has several built-in primitive types:
...
Nyx has no null values. Use Option types instead:
...
Nyx supports refinement types for more precise specifications (planned feature):
