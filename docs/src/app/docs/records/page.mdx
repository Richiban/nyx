...
Tuples are simply records with anonymous (positional) fields:
...
Positional and named fields can appear in the same object, but positional fields must always come first:

```nyx
def myObject = ("a", "b", someField = "c")

def badObject = ("a", "b", someField = "c", "d") -- Error: positional field after named
```
