...
Nyx uses clean, minimal syntax with significant indentation. Most expressions are composable, and the language prioritizes readability over brevity.
...
Nyx supports concise syntax for common lambda patterns:
...
Everything in Nyx is an expression that returns a value:
...
Nyx uses significant indentation for blocks:
